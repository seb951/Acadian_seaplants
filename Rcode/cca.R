...
#    as.data.frame(out)
  ...
  #save candidates
  if(biotope == "soil") write.table(candidate.ASV.taxo,paste("results/candidate.ASV.f",spp,"s.txt",sep=""),row.names = T, col.names = T, quote = T)
  if(biotope == "root") write.table(candidate.ASV.taxo,paste("results/candidate.ASV.f",spp,"r.txt",sep=""),row.names = T, col.names = T, quote = T)
  
#  if(biotope == "root" & spp == "f") dev.print(device=pdf,"figures/fungi/Figure5_f_RDA.pdf", onefile=FALSE)
#  if(biotope == "root" & spp == "b") dev.print(device=pdf,"figures/bacteria/Figure6_b_RDA.pdf", onefile=FALSE)
